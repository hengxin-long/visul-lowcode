import type { PropObj } from "@/types/form";

/** 公共只读属性 */
const commonReadonlyAttr: PropObj = {
  label: "是否只读",
  prop: "readonly",
  type: "switch",
};
...
export const componentAttrConfig: Record<string, PropObj[]> = {
  ...
  button: [
    ...
    { label: "自定义按钮颜色（rgb或十六进制颜色）", prop: "color", type: "input" },
    { label: "宽度，输入带后缀%或px的数", prop: "width", type: "input" },
  ],
  title: [
    { label: "标题级别", prop: "level", type: "number", limit: { min: 1, max: 6 } },
    { label: "标题内容", prop: "innerText", type: "input" },
    { label: "字体颜色（rgb或十六进制颜色）", prop: "color", type: "input" },
    { label: "字体位置（left/center/right）", prop: "textAlign", type: "input" },
    { label: "字体加粗（400不加粗 700加粗）", prop: "fontWeight", type: "input" },
  ],
  checkboxGroup: [
    ...commonBaseAttr,
    { label: "渲染选项的组件类型", prop: "type", type: "groupEnum" },
    { label: "可被勾选的 checkbox 的最小数量", prop: "min", type: "number", limit: { min: 0 } },
    { label: "可被勾选的 checkbox 的最大数量", prop: "max", type: "number", limit: { min: 0 } },
    { label: "多选框", prop: "options", type: "options" },
  ],
  radioGroup: [
    ...commonBaseAttr,
    { label: "渲染选项的组件类型", prop: "type", type: "groupEnum" },
    { label: "按钮形式的单选框激活时的文本颜色", prop: "text-color", type: "input" },
    { label: "按钮形式的单选框激活时的填充色和边框色", prop: "fill", type: "input" },
    { label: "单选框", prop: "options", type: "options" },
  ],
};
