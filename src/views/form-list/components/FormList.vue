  ...
  <div class="form-body">
    <div class="form-nav">
      <div class="nav">
        <ul>
          <li
            v-for="item in selectStatusBtn"
            :key="item.id"
            :class="item.type"
            @click="handleStatusForm(item.type)"
          >
    ...
    <div class="form-table">
      ...
      <!-- 浏览表单弹窗 -->
      <el-dialog
        v-model="isBrowseFormVisible"
        title="浏览表单"
        width="500"
        :before-close="closeBrowseForm"
      >
      ...
      <el-dialog
        class="export"
        v-model="isExportForm"
        title="导出json表单"
        width="300"
        :before-close="closeExportForm"
      >
        <h4>设置文件名</h4>
...
<script setup lang="ts">
  ...
  const browseForm = (row: FormItem) => {
    ...
    toEditForm.value = row;
    switchPreview();
    getFormDetail(toEditForm.value.id)
      .then(
        (resolve) => {
